...
function reportConflicts(conflicts) {
  ...
  for (const c of conflicts) {
    console.error(
      `  ${c.version}  ${c.filename}: the file no longer matches the recorded checksum.`,
    );
  }
  console.error("");
...
if (flags.status) {
  ...
  console.log(
    `  pending migrations:    ${pending.length}${pending.length ? ` (${pending.map((m) => m.version).join(", ")})` : ""}`,
  );
  console.log(
    `  checksum conflicts:    ${conflicts.length}${conflicts.length ? ` (${conflicts.map((c) => c.version).join(", ")})` : ""}`,
  );
  console.log(
    `  adoption required:     ${!ledgerPresent || rows.length === 0 ? "yes, if this is an existing AIRS Agent database (npm run db:migrate:adopt)" : "no"}`,
  );
...
if (flags["adopt-existing"]) {
  ...
  if (rows.length > 0) {
    fail(
      `The migration ledger already contains ${rows.length} row(s); adoption is not required.`,
      4,
    );
  }
  ...
  if (!gis.installed)
    fail(
      `Adoption aborted: PostGIS is not installed in this database.\n${POSTGIS_MISSING_ERROR}`,
      7,
    );
  ...
  if (!probe.ok)
    fail(`Adoption aborted: could not probe the canonical schema.\n${probe.stderr}`, 4);
  ...
  if (canonical.missing.length) {
    fail(
      [
        ...
        "",
        "Run `npm run db:reconcile-legacy` first. Nothing was recorded.",
      ].join("\n"),
      4,
    );
  }
  ...
  const suite = runSqlSuite({
    run: (sql) => run(sql, { capture: true }),
    root: REPO_ROOT,
    onFile: (file) => console.log(`  ok  ${file}`),
  });
  if (!suite.ok)
    fail(
      `Adoption aborted: verification suite failed (${suite.failedFile}). Nothing was recorded.\n${suite.stderr}`,
      4,
    );
  ...
  if (!security.ok)
    fail(
      `Adoption aborted: security verification failed. Nothing was recorded.\n${security.stderr}`,
      4,
    );
  ...
  if (!platform.ok)
    fail(
      `Adoption aborted: platform verification failed. Nothing was recorded.\n${platform.stderr}`,
      4,
    );
  ...
  if (!result.ok)
    fail(
      `Adoption aborted: schema verification failed. Nothing was recorded.\n${result.stderr}`,
      4,
    );
  console.log("");
  console.log("Adopted the existing database. Migrations recorded WITHOUT being executed:");
  for (const m of migrations)
    console.log(`  ${m.version}  ${m.filename}  sha256:${m.checksum.slice(0, 12)}...`);
  console.log(
    `Total: ${migrations.length} migrations. Run \`npm run db:migrate\` to confirm zero pending.`,
  );
  process.exit(0);
}
...
if (flags["dry-run"]) {
  ...
  console.log(`  execution path: ${pathLabel}`);
  console.log(
    `  applied (${applied.length}): ${applied.map((m) => m.version).join(", ") || "none"}`,
  );
  console.log(
    `  pending (${pending.length}): ${pending.map((m) => m.version).join(", ") || "none"}`,
  );
  console.log(
    `  checksum conflicts (${conflicts.length}): ${conflicts.map((c) => c.version).join(", ") || "none"}`,
  );
  process.exit(conflicts.length ? 3 : 0);
}
...
if (pending.length === 0) {
  console.log(
    `Zero pending migrations. ${applied.length} already applied (execution path: ${pathLabel}).`,
  );
  process.exit(0);
}
...
for (const migration of pending) {
  ...
  if (result.stderr.includes(MARKER_ALREADY_APPLIED)) {
    console.log(
      `  -- ${migration.version} was applied by a concurrent runner; skipped, nothing changed.`,
    );
    continue;
  }

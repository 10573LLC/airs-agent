...
function DesktopConsole({ projection, mapItems }: { projection: SimOperationalProjection; mapItems: MapLayerItem[] }) {
  return (
    <div className="hidden xl:grid xl:grid-cols-[minmax(0,0.95fr)_minmax(640px,1.35fr)] xl:gap-4">
      ...
      <MapView projection={projection} mapItems={mapItems} desktop />
    </div>
  );
}
...
function DesktopPanel({ title, description, children }: { title: string; description: string; children: ReactNode }) {
  return (
    <SectionCard title={title} description={description} className="h-[21.5rem] min-w-0 overflow-hidden">
      <div className="h-[15.5rem] overflow-y-auto pr-1">{children}</div>
    </SectionCard>
  );
}

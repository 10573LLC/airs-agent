    ...
    async (_ctx, q) =>
      q.query<MemberRow>(
        `SELECT m.id AS "membershipId", m.user_id AS "userId", m.account_id AS "accountId",
                ...
                m.role_key AS "roleKey", m.status, to_json(m.activated_at)#>>'{}' AS "activatedAt"
    ...
    async (_ctx, q) =>
      q.query<{
        ...
        `SELECT a.id::text AS id, a.action, a.resource_type AS "resourceType", a.outcome,
                to_json(a.occurred_at)#>>'{}' AS "occurredAt", u.display_name AS actor, a.detail::text AS detail

 ...
 */
export async function cleanupRunFixtures(admin: Client, scope: RunFixtureScope): Promise<void> {
  ...
  try {
    ...
    await run(
      `DELETE FROM airs.incidents WHERE created_by = ANY($1::uuid[]) OR org_id = ANY($2::uuid[])`,
      [a[2], a[1]],
    );
